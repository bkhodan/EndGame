...
Object * updateAsteroids(Object * asteroids, SDL_Texture * image)
{
    ...
    if((rand() % (int) (30 / ((ASTEROID_SPEED < 30) ? ASTEROID_SPEED : 30))) == 0)
    {
        ...
        if(random >= 3)
        {
            asteroid = createObject(image, 0, 1, ASTEROID_SMALL, 2, 0, 64, 32, 32, 1.0);
        }
        else if(random >= 1)
        {
            asteroid = createObject(image, 0, 1, ASTEROID_MEDIUM, 5, 32, 64, 64, 64, 1.0);
        }
        else
        {
            asteroid = createObject(image, 0, 1, ASTEROID_LARGE, 8, 96, 64, 96, 96, 1.0);
        }
...
Object * updateUserBullets(Object * ship, Object * bullets, SDL_Texture * image, uint32_t * timer)
{
    ...
    if(timer[BULLET_TINY_TIMER] < SDL_GetTicks())
    {
        ...
        {
            bullet = createObject(image, 0, 2, BULLET_TINY, 1, 0, 176, 16, 16, 1.0);
